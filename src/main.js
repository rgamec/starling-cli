...
export async function returnDailySpendForDate(config, dateToCheckStr){

    const account = config.get('accounts')[0]; // Assume only one account
    var dateToCheck = new Date(dateToCheckStr);
    ...
    try {
        ...
        process.stdout.write(String(dateToCheck.getFullYear()) + '-' + String(dateToCheck.getMonth()+1) + '-' + String(dateToCheck.getDate()) + ',');
...
export async function returnDailySpendForCurrentMonth(config) {
    ...
    // Iterate through each day of the month until the current day
    var currentDate = new Date();
    var dateToCheck = new Date(currentDate.getFullYear() + '-' + (currentDate.getMonth()+1) + '-1');

    for (var i = 1; i <= currentDate.getDate(); i++){
        
        var dateToCheckString = dateToCheck.getFullYear() + '-' + String(dateToCheck.getMonth()+1) + '-' + String(dateToCheck.getDate());
        await returnDailySpendForDate(config, dateToCheckString);
